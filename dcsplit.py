...
def add_label_funcData(seg_start, seg_end, addr, short):
    labelled_addresses.add(addr)
    ...
    if label_candidate(integer) == True:
        if integer < seg_start or integer >= seg_end:
            need_export.add(integer)
...
if 'blacklist_code' in dcsplit_config:
    for elem in dcsplit_config['blacklist_code']:
        # hack, we dont know the segstart or segend, so 0,0 does for now
        add_label_funcData(0, 0, elem, True)
...
def is_nop_padding(address, offset):
    code = get_short_at(offset)
    # check if yaml is in "weird big endian nop" mode
    if big_endian_nop_padding:
        return code[0] == 0x0900
    else:
        return sh4.decode(code[0], address).op == sh4.OP.NOP

# Calls callback for every instruction in the specified code section
def disasm_iter(seg_start, seg_end, offset, address, size, callback):
    if size == 0:
        return
    start = address
    end = address + size
...
def bss_iter(offset, address, size):
    ...
    # get the labels for this section
    sorted_bss_labels = list(filter(lambda x: x >= address and x < address + size, list(sorted(labelled_addresses))))
    
    # if there's a label right on the start of the section, properly label it
    # if not then make sure there's proper amount of padding before the first label
    index = -1
    if address in sorted_bss_labels:
        index = 0

    while index < len(sorted_bss_labels):
        if index == -1:
            write_asm_file_output(f"\t.res.b {sorted_bss_labels[0] - address}")
        else:
            address = sorted_bss_labels[index]

            # handle "last label" case
            if index < len(sorted_bss_labels) - 1:
                next_address = sorted_bss_labels[index + 1]    
            else:
                next_address = address + size
    
            write_asm_file_export("\n\t.EXPORT %s" % addr_to_label_noh(address))
            write_asm_file_output("%s:" % addr_to_label_noh(address))
            write_asm_file_output(f"\t.res.b {next_address - address}")
        index += 1
...
def disassemble_callback(seg_start, seg_end, address, offset, insn, bytes):
    ...
    if len(insn.operands) > 0 and insn.operands[0][0] == sh4.OPER_TYPE.ADDRESS:
        ...
        if addr in labelled_addresses and (addr < seg_start or addr >= seg_end):
            write_asm_file_import("\t.IMPORT %s" % (addr_to_label_noh(addr)))    
    ...
    # Output label (if any)
    if address in labelled_addresses:        
        if address in need_export or address in label_names:
            write_asm_file_export("\t.EXPORT %s" % addr_to_label_noh(address))
        write_asm_file_output("%s:" % addr_to_label_noh(address))
...
def func_search(addr, seg_end):
    ...
    if not is_nop_padding(max_end + 2 + key, max_end + 2) and max_end + 2 not in is_data:
        late_data = True
        return max_end + 2
...
def get_label_callback(seg_start, seg_end, address, offset, insn, bytes):
    ...
    if insn.op in {sh4.OP.BF, sh4.OP.BRA, sh4.OP.BRAF, sh4.OP.BSR, sh4.OP.BSRF, sh4.OP.BT}:
        for op in insn.operands:
            if op[0] == sh4.OPER_TYPE.ADDRESS:
                if op[1] < seg_start or op[1] > seg_end:
                    need_export.add(op[1])               

                add_label(op[1], None)

    if insn.op == sh4.OP.MOV or insn.op == sh4.OP.MOVA:
        if insn.operands[0][0] == sh4.OPER_TYPE.ADDRESS:
            if insn.operands[0][1] < seg_start or insn.operands[0][1] > seg_end:
                need_export.add(insn.operands[0][1])            

            if insn.length_suffix == sh4.SUFFIX.L or insn.op == sh4.OP.MOVA:
                add_label_funcData(seg_start, seg_end, insn.operands[0][1], False)
            else:
                add_label_funcData(seg_start, seg_end, insn.operands[0][1], True)
...
for i in range(len(segs)):
    ...
    if seg['type'] == 'code':
        ...
        for j in range(len(subsegs)):
            ...
            # if the segment doesn't have code (currently, valid segments are asm, libasm, hasm)
            if 'asm' not in subseg[1]: 
                continue
...
def code_interval_step(callback):
    for interval in code_intervals:
        ...
        disasm_iter(start + key, end + key, start, start + key, end - start, callback)
...
def gen_asm(object_name, seg_type, start, end, seg_start=None, seg_end=None):
    # we need the start/end, seg_start/end distinction for per-function
    # disassemblies to understand not to import labels from functions in the same file
    ...
    if('asm' in seg_type):
        disasm_iter(seg_start + key, seg_end + key, start, start + key, end - start, disassemble_callback)
    elif seg_type == 'bss':
...
for i in range(len(segs)):
    ...
    gen_asm(
        dcsplit_config['asm_path'] + "/" + dir + seg['name'], 
        seg['type'], 
        start, 
        end
    )
